...
## 11. Tech-review resolutions (Opus, 2026-07-01) — these GOVERN the plan

**Verdict: sound with changes.** Anchors: world-tick every `DEFAULT_WORLD_TICK_INTERVAL=5.0s`; breaths ~1-3s (Gemini). Value energy≈materials 1:1 (`HEARTH_ENERGY_PER_MATERIAL=1.0`).

**4 MANDATORY changes (bake into the plan):**
1. **`Home.last_integrity_at` (fixes a decay-acceleration death-spiral bug).** Seed `= built_at` in `build_home`; advance to `now` on EVERY tick (covered AND missed); compute BOTH repair and decay from `elapsed = now - last_integrity_at`. Keep `last_upkeep_at` SOLELY for `owed`/arrears. (Using `now - last_upkeep_at` for decay accelerates it every missed tick → collapse ~2.5× too fast, unbounded.)
2. **Thieve leaves the home STANDING at integrity ~0 with the vault ZEROED — do NOT immediately make_ruin.** Never feed a split vault into a remnant (double-count → mint). A later collapse computes `remnant = FRACTION*(BUILD_COST + 0)`, no hazard.
3. **Colonize resolves at-most-one-home:** enroll ONLY currently-homeless breachers (`stakeholder_home_of is None`) as new stakeholders; others merely participated; if the final striker already holds a home, auto-detach it (`remove_stakeholder`) or block that raider as colonizer. Explicit, not incidental.
4. **Status-guard EVERY existing home op against RUIN** (STANDING-only): the upkeep sweep, `pledge_home`, `use_hearth`, `deposit_to_home`/`withdraw_from_home`, `leave_home`, `modify_home_integrity`. Else "pledge to a ruin"/"hearth in a ruin"/sweep-decays-a-ruin.

**Fork resolutions:**
- **A — incremental repair/decay (from `last_integrity_at`):** Covered branch → `modify_home_integrity(+HOME_REPAIR_PER_SECOND*elapsed)` (auto-clamps to `max_integrity(s)`), pay upkeep, advance `last_upkeep_at`. Missed branch → `modify_home_integrity(-HOME_DECAY_PER_SECOND*elapsed)`, freeze `last_upkeep_at`. Dials: `HOME_DECAY_PER_SECOND=2.0` (=`HOME_DECAY_PER_MISSED_TICK/DEFAULT_WORLD_TICK_INTERVAL`=10/5 → reproduces −10/tick, preserves broke-home wall-time: M(1) collapses in 50s), `HOME_REPAIR_PER_SECOND=10.0` (+50/covered-tick; LIVE dial). **Non-regression is structural:** a funded home takes the covered branch every tick → never enters decay → clamped at `max_integrity(s)`, rate-independent. Update `tests/world/tick_test.py` covered/missed expectations to time-based.
- **B — break-in dials:** keep `BREAKIN_INTEGRITY_DAMAGE=25, BREAKIN_ENERGY_COST=15, BREAKIN_MATERIALS_COST=10` (per-attempt cost 25 mat-equiv). **Coordination is DISCRETE (repair is lumpy, not continuous):** net progress needs `Σ(break_ins in one 5s window)*25 > HOME_REPAIR_PER_SECOND*5 = 50` → **>2 break_ins/window.** A fast lone raider CAN solo-crack M(1)=100 (4-5 attempts vs zero mid-window healing) but at ruinous self-cost (~75 energy+50 materials → near-paralysis) — this is "a lone shelter is soft," acceptable; what forces coordination for M≥150 is the per-attempt resource burn (self-limiting like `attack`). Balance: typical vaults (<300) net-negative to breach at every tier; only hoard-tier (~300) is worth a coordinated raid after the split + repair leakage. LIVE-run targets: `BREAKIN_*` + `HOME_REPAIR_PER_SECOND`; fallback `BREAKIN_INTEGRITY_DAMAGE→15` if solo-raiding of M(1) dominates.
- **C — tool interface:** `break_in(target_home, intent∈{thieve,colonize})`; pre-breach attempts just damage+cost; the blow crossing integrity ≤0 executes that attempt's intent ATOMICALLY (avoids the race where the next covered tick repairs above 0 before a separate thieve lands; matches `attack` looting in-call). Final striker's intent wins.
- **D — breachers reset:** clear `breachers` when integrity repairs back to `max_integrity(s)` (sufficient; the thieve/colonize co-located+ALIVE filter already closes the stale-breacher exploit). No time-expiry in v1.
- **E — thieve aftermath:** see Mandatory #2 (standing-at-0, vault zeroed).
- **F — raider perception:** extend `look_around` so for each co-located home the being is NOT a stakeholder of, it shows id/owner/**integrity(soundness)** + a **hoard FLAG** (from `home_is_hoarding`) — NOT the exact vault number; exact vault stays own-home-only (2b). DD9-safe; reuse `homes_in_region` filtered by `not is_stakeholder`.
- **G — see decomposition + conservation below.**

**DECOMPOSITION → 7 ordered, independently-testable tasks:**
1. **Incremental-repair switch** (foundational/riskiest): `last_integrity_at` + `HOME_REPAIR_PER_SECOND=10.0`/`HOME_DECAY_PER_SECOND=2.0` + rewrite covered/missed branches + update tick tests. (§12: observe stability before break-in.)
2. **Home model + RUIN status guards:** `HomeStatus{STANDING,RUIN}`, `ruined_at`, `remnant_materials`, `breachers`; make every existing home op STANDING-only (no behavior change — all homes STANDING; guard tests).
3. **break_in + breach:** tool + `record_breacher`/`clear_breachers` + co-location/non-stakeholder/STANDING guards + damage + pure-sink cost + breach-at-≤0 + clear-on-full-repair.
4. **Thieve/colonize** (atomic on the crossing blow): thieve split (=vault, zero vault, standing-at-0); `colonize_home` reassign/evict/retain-vault + at-most-one-home resolution.
5. **Ruins:** `make_ruin` (remnant=`FRACTION*(BUILD_COST+vault)` then zero/clear), `scavenge_ruin`; repurpose collapse→`make_ruin`; ruin-sweep after `RUINS_PERSIST_SECONDS=120.0` (mirror corpse sweep); `scavenge_ruins` tool (deduct-remnant-first). Assert `RUINS_SCAVENGE_FRACTION=0.5 < 1`.
6. **Carried 2b + visibility:** `(hoarding)` markers on BOTH homes + agents world-table sections; raider perception (Fork F); status/breachers/remnant columns; feed verbs; WORLD_MECHANICS DD9 prose.
7. **Conservation property test + LIVE tuning run:** extend the existing property test (`tests/tools/homes_test.py`) to include `remnant_materials` and drive the FULL raid lifecycle (build → deposit-to-hoard → coordinated break_in×N → thieve → make_ruin → scavenge → sweep), asserting total materials strictly non-increasing (only region regen raises it) + energy never minted. Then the LIVE Gemini-regime run to tune the dials (SURFACE TO SAFI).

**CONSERVATION/STABILITY:** build→collapse→scavenge farm blocked by `FRACTION<1` (build 80 → recover 40, net loss); mega-fortress blocked by the 200 ceiling + now-cumulative break-in; clutter blocked by the `RUINS_PERSIST_SECONDS` sweep; war-of-extinction blocked by loot<cost + per-attempt burn. The vault→remnant carry CLOSES 2b's collapse-destroys-vault leak (converts an untraceable loss into a partial perceivable scavengeable remnant + a deliberate sink).

**New WorldState:** `record_breacher`, `clear_breachers`, `colonize_home`, `make_ruin`, `scavenge_ruin`; `HomeStatus` enum; `Home` += `status/ruined_at/remnant_materials/breachers/last_integrity_at`. **Events (LOCAL):** `home_breached`, `home_thieved`, `home_colonized`, `ruins_scavenged`, repurposed `home_collapsed`→ruin. **Constants asserts:** `RUINS_SCAVENGE_FRACTION < 1`.
