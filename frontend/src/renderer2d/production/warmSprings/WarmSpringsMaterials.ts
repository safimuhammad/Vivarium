/**
 * The Warm Springs material vocabulary — production.
 *
 * Promoted verbatim (list, priority order, blocking set, variant counts) from the
 * approved pilot `qa/warmSpringsPilot/springsMaterials.ts`, because those tables are
 * the coordination point with the published `warm-springs-v1` art: a drift here asks
 * the atlas for a frame nobody drew.
 *
 * Priority IS the paint order. The lowest-priority material among a tile's four
 * corners fills the tile; every higher material is laid over it as a corner-masked
 * transition frame. That stack is what makes pool, sinter, travertine, ochre mat and
 * sward interlock instead of meeting at a hard 32 px tile border.
 *
 * `sinter` and `travertine` are WALKABLE. That single decision is what lets Warm
 * Springs carry its geothermal identity *inside* the settlement, on ground beings
 * stand on, at zero shelter-plot cost — see `.superpowers/sdd/warm-springs-pilot-report.md` §6.
 */
