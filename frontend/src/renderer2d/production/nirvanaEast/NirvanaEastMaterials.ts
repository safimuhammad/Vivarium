/**
 * The Nirvana East material vocabulary — production.
 *
 * Promoted verbatim (list, priority order, blocking set, variant counts) from the
 * approved pilot `qa/nirvanaEastPilot/eastMaterials.ts`, because those tables are the
 * coordination point with the published `nirvana-east-v1` art: a drift here asks the
 * atlas for a frame nobody drew. Mirrors `warmSprings/WarmSpringsMaterials.ts` exactly
 * in shape — priority is the paint order, the lowest-priority material among a tile's
 * four corners fills the tile, and every higher material is laid over it as a
 * corner-masked transition frame.
 *
 * Names are promoted to the `NIRVANA_EAST_*` / `nirvanaEast*` production convention
 * wherever the pilot used a bare name, so this module's exports never collide with a
 * sibling region's same-shaped material module in a shared import scope (unlike
 * `WarmSpringsMaterials.ts`, which is the only material module its own package tree
 * ever imports at once, `NirvanaEastAtlas.ts` re-exports these alongside other
 * regions' equivalents, so the prefix is load-bearing here, not decoration).
 */
...
export const NIRVANA_EAST_GROUND_FAMILY: readonly NirvanaEastMaterial[] = Object.freeze([
  "sand",
  "redsand",
  "oxide",
  "hardpan",
...
export const NIRVANA_EAST_WATER_FAMILY: readonly NirvanaEastMaterial[] = Object.freeze([
  "brine",
  "brinerim",
]);

/** Bank materials that own a shoreline / wet-line overlay set (contract §4). */
export const NIRVANA_EAST_SHORE_MATERIALS: readonly NirvanaEastMaterial[] = Object.freeze([
  "salt",
  "hardpan",
  "oxide",
]);
...
export const NIRVANA_EAST_CLIFF_FAMILY: readonly NirvanaEastMaterial[] = Object.freeze([
  "mesa",
  "scarp",
  "slot",
]);
...
export function nirvanaEastBaseFrameId(material: NirvanaEastMaterial, variant: number): string {
 ...
 */
export function nirvanaEastEdgeFrameId(
  material: NirvanaEastMaterial,
  mask: number,
  variant: number,
): string {
 ...
 */
export function nirvanaEastShoreFrameId(
  material: NirvanaEastMaterial,
  mask: number,
  variant: number,
): string {
...
export function isWaterMaterial(material: NirvanaEastMaterial): boolean {
  return NIRVANA_EAST_WATER_FAMILY.includes(material);
}

/** True when `material` owns a shoreline set. */
export function hasShoreSet(material: NirvanaEastMaterial): boolean {
  return NIRVANA_EAST_SHORE_MATERIALS.includes(material);
}

/** True when `material` blocks standing/routing. */
export function isBlockingNirvanaEastMaterial(material: NirvanaEastMaterial): boolean {
