/**
 * Renders a real "before" baseline of the CURRENT production `nirvana_east`
 * region at native 1:1 scale (3072x3072 px, 96x96 tiles @ 32px).
 *
 * `nirvana_east` uses the GENERIC region-map recipe path (there is no bespoke
 ...
 *
 *   1. Builds the REAL `RegionMapIdentity` for `nirvana_east` from the actual
 ...
 *   4. Reads the REAL asset pack for the `dry-scrub` kit via the REAL,
...
export async function renderNirvanaEastBaseline(outputRoot = DEFAULT_OUTPUT) {
  ...
  try {
    ...
    const nirvanaEast = regions.find((region) => region.name === REGION_ID);
    if (nirvanaEast === undefined) {
      throw new Error(`config/world.yaml has no region named ${REGION_ID}.`);
    }

    const identity = identityModule.createRegionMapIdentity(RUN_SEED, nirvanaEast, regions);
    if (identity.archetype !== "dry_scrub") {
      throw new Error(`Expected nirvana_east archetype dry_scrub, got ${identity.archetype}.`);
    }
    const recipe = recipeModule.createRegionMapRecipe(identity);
    if (recipe.kit !== "dry-scrub") {
      throw new Error(`Expected nirvana_east kit dry-scrub, got ${recipe.kit}.`);
    }
    const pack = manifestModule.requireRegionAssetPack(recipe.kit);

    const assetRoot = path.join(FRONTEND_ROOT, "src/assets/renderer2d/regions/dry-scrub");
    const atlasSurfaces = {
      "dry-scrub-terrain": await loadSurface(path.join(assetRoot, "terrain.png")),
      "dry-scrub-scenery": await loadSurface(path.join(assetRoot, "scenery.png")),
      "dry-scrub-landmarks": await loadSurface(path.join(assetRoot, "landmarks.png")),
    };
    ...
    const outputFile = path.join(outputRoot, "00-nirvana-east-current-baseline.png");
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  renderNirvanaEastBaseline(outputRoot)
    .then((result) => {
