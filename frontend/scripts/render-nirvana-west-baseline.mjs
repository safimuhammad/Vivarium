/**
 * Renders a real "before" baseline of the CURRENT production `nirvana_west`
 * region at native 1:1 scale (3072x3072 px, 96x96 tiles @ 32px).
 *
 * `nirvana_west` uses the GENERIC region-map recipe path (there is no bespoke
 ...
 *
 *   1. Builds the REAL `RegionMapIdentity` for `nirvana_west` from the actual
 ...
 *   4. Reads the REAL asset pack for the `ash-waste` kit via the REAL,
...
export async function renderNirvanaWestBaseline(outputRoot = DEFAULT_OUTPUT) {
  ...
  try {
    ...
    const nirvanaWest = regions.find((region) => region.name === REGION_ID);
    if (nirvanaWest === undefined) {
      throw new Error(`config/world.yaml has no region named ${REGION_ID}.`);
    }

    const identity = identityModule.createRegionMapIdentity(RUN_SEED, nirvanaWest, regions);
    if (identity.archetype !== "ash_waste") {
      throw new Error(`Expected nirvana_west archetype ash_waste, got ${identity.archetype}.`);
    }
    const recipe = recipeModule.createRegionMapRecipe(identity);
    if (recipe.kit !== "ash-waste") {
      throw new Error(`Expected nirvana_west kit ash-waste, got ${recipe.kit}.`);
    }
    const pack = manifestModule.requireRegionAssetPack(recipe.kit);

    const assetRoot = path.join(FRONTEND_ROOT, "src/assets/renderer2d/regions/ash-waste");
    const atlasSurfaces = {
      "ash-waste-terrain": await loadSurface(path.join(assetRoot, "terrain.png")),
      "ash-waste-scenery": await loadSurface(path.join(assetRoot, "scenery.png")),
      "ash-waste-landmarks": await loadSurface(path.join(assetRoot, "landmarks.png")),
    };
    ...
    const outputFile = path.join(outputRoot, "00-nirvana-west-current-baseline.png");
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  renderNirvanaWestBaseline(outputRoot)
    .then((result) => {
