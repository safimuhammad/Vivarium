/**
 * Publish the approved Nirvana East "Butte Country" (composition B) vocabulary as the
 * production `nirvana-east-v1` art generation.
 *
 * The owner approved composition B on the pilot's own art
 * (`.superpowers/sdd/nirvana-east-pilot-report.md`, round 2 pass 2). That art sits at
 * 148,875 B = 75.72 % of the 196,608 B per-kit ceiling with ~47 KB of headroom, and the
 * pilot report is explicit that the headroom is not to be spent carelessly. So this script
 * does not re-author a single pixel: it republishes the pilot's exact `terrain.png` /
 * `scenery.png` bytes, verified against the sha256 the pilot manifest already declares,
 * and rewrites only the MANIFEST — adding the `compressedBytes` / `decodedBytes` fields
 * the production asset descriptors require, which the pilot manifest expressed as
 * `bytes` / `rgbaBytes`.
 *
 * Byte-identical art is the point: what ships is exactly what was approved by eye, and the
 * mesa read took two rounds to win.
 *
 * Publication is atomic and content-addressed, exactly as
 * `build-warm-springs-v1-atlas.mjs` does it — staged in a `mkdtemp`, renamed into
 * `frontend/src/assets/renderer2d/regions/.nirvana-east-v1-generations/<fingerprint>/`,
 * with a symlink swapped over `frontend/src/assets/renderer2d/regions/nirvana-east-v1`.
 * The `dry-scrub` biome kit, `nirvana-v2`, `nirvana-v3` and `warm-springs-v1` are never
 * read or written. Until the profile is flipped, this generation is INERT: nothing
 * imports it.
 *
 * Usage: `node scripts/build-nirvana-east-v1-atlas.mjs`
 */
...
const SOURCE_DIRECTORY = path.join(FRONTEND_ROOT, "src/qa/nirvanaEastPilot/assets");
 ...
 *
 * Only the two `images` records change. Every other field — the frame ids, the terrain
 * grid, the scenery tuples, the corner-bit declaration, the material vocabulary and the
 * per-material variant counts — is passed through untouched, because changing any of them
...
function productionManifest(pilot, terrainBytes, sceneryBytes) {
  const image = (record, bytes, label) => {
    ...
    if (record.sha256 !== digest) {
      throw new Error(
        `Nirvana East ${label} art does not match the sha256 its pilot manifest declares `
        + `(manifest ${record.sha256}, file ${digest}). The approved art has drifted; stop.`,
      );
    }
    if (record.bytes !== bytes.length) {
      throw new Error(
        `Nirvana East ${label} manifest byte count ${record.bytes} does not match the `
...
async function publishAtomically(files, outputDirectory) {
  ...
  if (JSON.stringify(names) !== JSON.stringify(expected)) {
    throw new Error(`Nirvana East publication requires exactly ${expected.join(", ")}`);
  }
...
async function main() {
  ...
  if (totalBytes > REGION_KIT_CEILING_BYTES) {
    throw new Error(
      `Nirvana East kit total ${totalBytes} B exceeds the ${REGION_KIT_CEILING_BYTES} B ceiling.`,
    );
  }
