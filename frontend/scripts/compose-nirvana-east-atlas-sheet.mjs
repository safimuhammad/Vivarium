/**
 * Composes the CONTACT SHEET for the Nirvana East pilot tile vocabulary.
 *
 * Sibling of `compose-warm-springs-atlas-sheet.mjs`. Nothing here invents
 * art: it reads `src/qa/nirvanaEastPilot/assets/` and blits the authored
 * frames at 1:1. The transition and shoreline strips are painted with the
 * SAME corner-sampled algorithm the scene painter will use, so the strips are
 * a real test of seam continuity, not a diagram of one.
 *
 ...
 *   C  SHORELINE        - the w.* sets on salt/hardpan/oxide against brine/brinerim
 *   D  SCENERY          - every sprite at 1:1 on a neutral mid-grey field
 *
 * Output: docs/frontend/mockups/regions/nirvana-east-pilot/atlas-contact-sheet.png
 */
...
const ASSETS = path.resolve(FRONTEND_ROOT, "src/qa/nirvanaEastPilot/assets");
...
const TILE = 32;
const BACKGROUND = [18, 16, 18, 255];
const NEUTRAL = [104, 100, 98, 255];
const LABEL = 20;
const PAD = 12;
  ...
  for (let ty = 0; ty < tilesY; ty += 1) {
    for (let tx = 0; tx < tilesX; tx += 1) {
      ...
      for (const material of present.slice(1)) {
        ...
        // the salt-efflorescence line, where a bank material meets a water tile
...
function label(text, x, y, size = 13, colour = "#f0e6dc") {
...
/** Section C: the shoreline sets, on each bank material, against both waters. */
...
function sectionShore(canvas, context, top) {
  ...
  const perRow = 2;
  label(
    "C. THE SALT LINE - w.* shoreline frames laid over the bank's own transition,"
    + " sharing its seed so the efflorescence ring follows exactly the same contour",
    PAD, top - 8, 15,
  );
  SHORE_CASES.forEach((pair, index) => {
    const [water, bank] = pair;
    const column = index % perRow;
    const row = Math.floor(index / perRow);
    const x = PAD + column * (width + PAD);
    const y = top + row * (height + LABEL + PAD);
    ...
    const cornerAt = (gx, gy) => (
      fbm(gx * 0.26 + 2.3, gy * 0.40 + 5.9, seed, 3) + gy * 0.045 > 0.50 ? bank : water
    );
    paintField(canvas, context, cornerAt, tilesX, tilesY, x, y, seed);
    label(`${bank} bank against ${water}`, x, y + height + 14, 12);
  });
...
export async function composeNirvanaEastAtlasSheet(outputRoot = OUTPUT_ROOT) {
  await mkdir(outputRoot, { recursive: true });
  const context = await loadAtlas();
  labels.length = 0;
  const width = 1120;
  const canvas = createCanvas(width, 4200, BACKGROUND);
