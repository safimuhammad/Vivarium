/**
 * Composes the CONTACT SHEET for the Nirvana West pilot tile vocabulary.
 *
 * Sibling of `compose-warm-springs-atlas-sheet.mjs`. Nothing here invents
 * art: it reads `src/qa/nirvanaWestPilot/assets/` and blits the authored
 * frames at 1:1, because a zoomed crop is not evidence.
 *
 * The transition and rim strips are painted with the SAME algorithm the
 * scene painter will use - sample the material field at tile CORNERS, fill
 * with the lowest-priority corner material, overlay every higher material
 * with the corner-mask frame keyed by which corners it owns, then lay the
 * rim frame where a bank material meets the void family. So the strips are a
 * real test of seam continuity, not a diagram of one.
 *
 ...
 *   C  RIM LINES        - the w.* sets on each bank material against the void family
 *   D  SCENERY          - every sprite at 1:1 on a neutral mid-grey field
 *
 * Output: docs/frontend/mockups/regions/nirvana-west-pilot/atlas-contact-sheet.png
 */
...
const ASSETS = path.resolve(FRONTEND_ROOT, "src/qa/nirvanaWestPilot/assets");
...
const TILE = 32;
const BACKGROUND = [14, 13, 18, 255];
const NEUTRAL = [96, 92, 102, 255];
const LABEL = 20;
const PAD = 12;
 ...
 * Corners are ordered NW, NE, SE, SW to match the author's bit convention
 * (1, 2, 4, 8).
 */
function paintField(canvas, context, cornerAt, tilesX, tilesY, dx, dy, seed) {
  ...
  const rimSet = new Set(atlas.rimMaterials);
  const voidSet = new Set(atlas.voidMaterials);
  ...
  for (let ty = 0; ty < tilesY; ty += 1) {
    for (let tx = 0; tx < tilesX; tx += 1) {
      ...
      for (const material of present.slice(1)) {
        ...
        // the rim line, where a bank material meets the void family
        if (mask !== 15 && rimSet.has(material) && voidSet.has(base)) {
          const rimVariant = Math.floor(hash2(tx, ty, seed + 29) * 2);
          drawFrame(canvas, context, `w.${material}.${mask}.${rimVariant}`, px, py);
        }
      }
    }
  }
...
/** Section C: the rim sets, on each bank material, against the void family. */
const RIM_CASES = Object.freeze([
  ["ember", "cinder"],
  ["emberdim", "cinder"],
  ["brine", "cinder"],
  ["ember", "ashpale"],
...
function sectionRim(canvas, context, top) {
  const tilesX = 14;
  const tilesY = 5;
  const width = tilesX * TILE;
  const height = tilesY * TILE;
  const perRow = 2;
  label(
    "C. RIM LINES - w.* frames laid over the bank's own transition, sharing its"
    + " seed so the rim follows exactly the same contour as the bank edge",
    PAD, top - 8, 15,
  );
  RIM_CASES.forEach((pair, index) => {
    const [voidMat, bank] = pair;
    const column = index % perRow;
    const row = Math.floor(index / perRow);
    const x = PAD + column * (width + PAD);
    const y = top + row * (height + LABEL + PAD);
    ...
    const cornerAt = (gx, gy) => (
      fbm(gx * 0.26 + 2.3, gy * 0.40 + 5.9, seed, 3) + gy * 0.045 > 0.50 ? bank : voidMat
    );
    paintField(canvas, context, cornerAt, tilesX, tilesY, x, y, seed);
    label(`${bank} bank against ${voidMat}`, x, y + height + 14, 12);
  });
  const rows = Math.ceil(RIM_CASES.length / perRow);
  return top + rows * (height + LABEL + PAD);
}
...
export async function composeNirvanaWestAtlasSheet(outputRoot = OUTPUT_ROOT) {
  await mkdir(outputRoot, { recursive: true });
  const context = await loadAtlas();
  labels.length = 0;
  const width = 1120;
  const canvas = createCanvas(width, 4400, BACKGROUND);

  let cursor = 40;
  cursor = sectionBases(canvas, context, cursor) + 34;
  cursor = sectionTransitions(canvas, context, cursor) + 34;
  cursor = sectionRim(canvas, context, cursor) + 34;
