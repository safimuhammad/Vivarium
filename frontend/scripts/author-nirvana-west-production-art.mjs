/**
 * Authors the PRODUCTION Nirvana West tile vocabulary — the ash-waste /
 * "nuclear wasteland, all but dead" region, composition **B, The Ember Rift**,
 * with the owner-approved **INDUSTRIAL** ruin kit.
 *
 * Promoted from `author-nirvana-west-pilot-art.mjs` (which is left untouched as
 * the record of what was approved). Three deliberate differences, and nothing
 * else:
 *
 *  1. **The "ancient" ruin kit is gone.** The owner chose industrial, so
 *     `monolith` / `slumpwall` / `boneforest` / `ashbarrow` are not authored.
 *     That returns ~10 KB of the per-kit budget.
 *  2. **The industrialness is pushed, per the owner's one change** — *"maybe a
 *     bit more aggressive on industrialness."* See the INDUSTRIAL COMPLEX block
 *     below for what was added and the anti-clutter rule it is built to.
 *  3. **The animated-environment sheet ships the two kinds the shipped
 *     `ash-waste` biome kit actually declares**, in kit order: `ember` at cells
 *     0–3 and `smoke-anchor` at cells 4–7 (`biomeKits.ts:163`, binding
 *     `productionManifest.ts:929-932`). The pilot's `ember-vent` and
 *     `heat-haze` are NOT shipped: binding is purely ordinal, so a four-kind
 *     sheet would draw ember-vent art wherever production asked for
 *     smoke-anchor.
 *
 * What it emits:
...
/**
 * The staging directory `build-nirvana-west-v1-atlas.mjs` publishes FROM.
 *
 * Authoring straight into it means the published generation can never drift from this
 * script: the staged art is a build product, not a hand-maintained directory.
 */
...
export const MATERIALS = Object.freeze([
  ...
  "ash",
  "ashpale",
  // Ground evidence, both deliberately WALKABLE. See the note below.
  "slab",
  "spoil",
  "clinker",
  "scree",
...
/**
 * GROUND EVIDENCE, at TERRAIN level rather than as sprites — the owner's change,
 * expressed where it survives downscaling.
 *
 * `slab` is poured concrete: the foundation of a building that is no longer there, and
 * the apron a plant was stood on. `spoil` is what the works left on the ground: tailings,
 * scorch and chemical staining. Both are WALKABLE, which is what lets them be as large
 * and as central as they need to be — an apron may run straight under a shelter plot
 * without costing it a single tile.
 *
 * They earn terrain status rather than decal status for one measured reason: the world
 * map drops anything under roughly 3-4 tiles, so a 2-tile concrete decal vanishes at
 * island scale while a 6-tile slab field does not. And they are the only RECTANGULAR
 * things in a region generated entirely from fracture noise — a rectangle is the one
 * shape a wasteland cannot make by itself, so it keeps reading as "somebody built here"
 * all the way down to the island silhouette, where the ruins themselves are below the
 * size floor and do not.
 */
const SLAB = Object.freeze([[74, 68, 80], [96, 90, 100], [120, 113, 124], [146, 139, 150]]);
const SLAB_JOINT = Object.freeze([56, 51, 62]);
/** Iron stain, NOT fire. Deliberately browner and darker than any EMBER entry: coral
 * is this region's single saturated accent and rebar bleed must never compete with it. */
const SLAB_RUST = Object.freeze([92, 62, 60]);
const SPOIL = Object.freeze([[24, 20, 28], [40, 34, 45], [58, 50, 62], [78, 68, 82]]);
const SPOIL_RUST = Object.freeze([[64, 44, 44], [86, 60, 54]]);
...
/**
 * `slab` — poured concrete, and the one RULED surface in the region.
 *
 * The texture does the work that the material's shape cannot: the corner-mask
 * transition machinery is deliberately organic and rounds every boundary (the pilot's
 * own §12a finding), so a slab FIELD will always have a slightly ragged outline. That is
 * fine for broken concrete, but it means the "this was cast, not weathered" reading has
 * to come from INSIDE the tile — hence expansion joints on a ~19 px lattice, a bay grid
 * an order of magnitude coarser than the grain, spalled patches, and rebar rust showing
 * through where the cover has gone. Learned the expensive way: an earlier version of this
 * material as a flat sprite decal with an eroded edge was indistinguishable from drifted
 * `ash`, because this region's ash tiers ARE large pale patches.
 */
function slabPixel(wx, wy, seed) {
  const wear = fbm(wx * 0.035, wy * 0.035, seed + 211, 3);
  const grain = hash2(chunky(wx), chunky(wy), seed + 217);
  let index = level(SLAB, 0.42 + wear * 0.46);
  if (grain > 0.78) index += 1;
  else if (grain < 0.16) index -= 1;
  // The bay grid. Coarse, straight, and the whole point.
  // Bay joints. They cannot be made to RULE across the region: each of the eight base
  // variants samples a different world-space window (`offsetX = variant * 61.5`), so a
  // periodic grid lands at a different phase per variant and the joints do not line up
  // tile to tile. That is a property of this tile system, not a bug here — the same
  // class of limit as the pilot's "a 32 px tile is too small to hold a patch of scoured
  // ground". What survives is cast concrete panelling, which is what it needs to be. The
  // period is kept coarse enough (~26 px, most of a tile) that it reads as SLABS rather
  // than as cobbles.
  const joint = Math.min(
    Math.abs(((wx % 26) + 26) % 26 - 13),
    Math.abs(((wy % 23) + 23) % 23 - 11.5),
  );
  if (joint < 0.6) return SLAB_JOINT;
  // A structural crack wandering off a joint.
  const crack = Math.abs(fbm(wx * 0.055, wy * 0.075, seed + 223, 2) - 0.5);
  if (crack < 0.011) return SLAB_JOINT;
  // Spalled cover with the rebar showing.
  if (wear > 0.82 && hash2(chunky(wx), chunky(wy), seed + 229) > 0.975) return SLAB_RUST;
  return step(SLAB, index);
}

/**
 * `spoil` — tailings, scorch and chemical staining: what the works left on the ground.
 *
 * Deliberately the DARKEST walkable tier in the region, so a spoil field reads as a
 * shadow on the plain rather than as another rubble type. Its rust bloom is sparse and
 * clustered rather than allover: a stain has a source, and an evenly rusty ground reads
 * as a texture instead of as a spill.
 */
function spoilPixel(wx, wy, seed) {
  const heap = fbm(wx * 0.030, wy * 0.030, seed + 241, 3);
  const grit = fbm(wx * 0.16, wy * 0.16, seed + 247, 2);
  let index = level(SPOIL, 0.30 + heap * 0.50 + (grit - 0.5) * 0.22);
  if (hash2(chunky(wx), chunky(wy), seed + 251) > 0.80) index -= 1;
  const bloom = fbm(wx * 0.045, wy * 0.060, seed + 257, 2);
  if (bloom > 0.78 && hash2(chunky(wx), chunky(wy), seed + 263) > 0.55) {
    return SPOIL_RUST[bloom > 0.86 ? 1 : 0];
  }
...
const SCENERY_SPECS = Object.freeze([
  ...
  // --- THE INDUSTRIAL RUIN KIT (owner-approved) ---------------------------
  ...
  //
  // THE ANTI-CLUTTER RULE THIS BLOCK IS BUILT TO. A junkyard reads as noise;
  // an industrial complex reads as industry. What makes the cooling towers
  // work is that one glance identifies them. So the push is NOT "more props":
  // it is three things that all ADD ORDER rather than density —
  //
  //   (a) SCALE CONTRAST — one structure that dominates the region.
  //       `powerhall` is 320 px wide, 1.7x the widest ruin and ten tiles of
  //       frontage. Nothing else competes with it, which is exactly what makes
  //       it legible: the eye gets one subject and a supporting cast.
  //   (b) CONNECTIVE TISSUE — `pipeline`, `fence` and `railspur` are LINEAR
  //       species whose frames join edge to edge at 32 px, so they are laid as
  //       RUNS, not scattered as objects. A line across a plain is the single
  //       cheapest, most legible "this was built" signal there is, and a line
  //       is the opposite of clutter: it organises everything it passes.
  //   (c) GROUND EVIDENCE — `slab`, `apron`, `spill`, `tailings`. Flat, low
  //       contrast, drawn UNDER everything. A concrete pad where a building
  //       used to be says "industrial complex" without adding a silhouette at
  //       all, and it is the only part of the push that survives to map scale.
  //
  // Everything here is deliberately LOW CONTRAST against the plain except the
  // powerhall and the tank farm. The region's picture is still dark ground,
  // bone ash and coral fire; industry is its ARCHITECTURE, not its palette.

  // The one dominant structure: a turbine hall. Sawtooth north-light roof, a
  // long rhythm of tall broken windows, and one end collapsed into its own
  // roof trusses. Read this and you have read the region.
  { id: "powerhall", width: 320, height: 208, variants: 2, pivot: [70, 168], ruin: true },
  // A bunded tank farm: squat cylinders, one split down the side and slumped.
  { id: "tankfarm", width: 184, height: 124, variants: 2, pivot: [44, 100], ruin: true },
  // CONNECTIVE TISSUE. 32 px frames that JOIN: laid as runs, never as singles.
  // 0 = east-west pipe on trestles, 1 = the same with a joint collar,
  // 2 = north-south (running away), 3 = a riser/valve terminating the run.
  { id: "pipeline", width: 32, height: 44, variants: 4, pivot: [16, 38], linear: true },
  // 0 = east-west fence run, 1 = north-south, 2 = a corner post with a gap.
  { id: "fence", width: 32, height: 30, variants: 3, pivot: [16, 28], linear: true },
  // 0 = east-west rails, 1 = north-south. Ground level, flat, non-blocking.
  { id: "railspur", width: 32, height: 18, variants: 2, pivot: [16, 16], linear: true, flat: true },
  // GROUND EVIDENCE is carried by the `slab` and `spoil` TERRAIN tiers, not by decals.
  // Three flat decal species (`slab` / `apron` / `spill`) were authored, judged at 1:1,
  // and then DELETED in favour of the terrain tiers: a 2-tile concrete decal vanishes at
  // island scale, and a pale organic decal is indistinguishable from this region's own
  // ash. The one piece of ground evidence that keeps a BODY — and therefore cannot be a
  // terrain tier — is the spoil heap:
  { id: "tailings", width: 96, height: 52, variants: 2, pivot: [40, 46] },
]);
...
/**
 * The kinds this sheet ships, IN THE ORDER THE SHIPPED BIOME KIT DECLARES THEM.
 *
 * `ash-waste` declares `["ember", "smoke-anchor"]` (`biomeKits.ts:163`) and the
 * production binding is purely ORDINAL — kind `i` owns cells `i*4 .. i*4+3`
 * (`productionManifest.ts:929-932`). There is no frame-name table to disagree
 * with. So the pilot's four-kind sheet CANNOT ship as authored: production would
 * ask for `smoke-anchor` and be handed the pilot's `ember-vent` art from cells
 * 4–7. The pilot's own report called the two extra kinds optional; they are
 * dropped, and the two that remain are the two the region has been declaring
 * since before the pilot existed.
 */
...
// ---------------------------------------------------------------------------
// THE INDUSTRIAL RUIN KIT - owner-approved, and the region's whole architecture
//
// The lore question the pilot raised has been ANSWERED: Safi approved the
// industrial kit, so this world's canonical word for Nirvana West - "a nuclear
// wasteland" - is load-bearing. The ancient kit is not authored here at all.
//
// Light is keyed UPPER-LEFT and thrown LOWER-RIGHT to match Nirvana's shipped
// scenery, because all four islands sit side by side on the world map. The
// BEARING agrees; the foreshortening is deliberately opened up (SHADOW_DY 0.14
// -> 0.32) because at Nirvana's true ratio a 100 px ruin lays its entire shadow
// into a 14 px band and is invisible at 1:1.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// THE INDUSTRIAL PUSH - "a bit more aggressive on industrialness"
//
// Everything below exists to make the plain read as A COMPLEX THAT DIED rather
// than as scattered wreckage. Three instruments, in descending order of how
// much work they do:
//
//   1. SCALE CONTRAST     - `powerhall` dominates; everything else supports.
//   2. CONNECTIVE TISSUE  - `pipeline` / `fence` / `railspur` are LINEAR: their
//                           frames join edge-to-edge at the 32 px tile pitch so
//                           they are laid as runs. A run organises a plain; a
//                           scatter clutters it.
//   3. GROUND EVIDENCE    - `slab` / `apron` / `spill` are flat, low-contrast
//                           decals with no silhouette at all. They say "there
//                           were buildings here" without adding one object to
//                           the skyline, and they are the only part of this
//                           block that survives to island scale.
//
// Palette discipline is unchanged: every pixel is a fixed entry from the ruin
// kits or the terrain palettes, alpha is quantised to the four contract levels,
// and no value is ever interpolated.
// ---------------------------------------------------------------------------

/** Lay a flat ground stain at one of the four contract alpha levels. */
function stain(surface, x, y, rgb, level) {
  if (level <= 0) return;
  setPixel(surface, x, y, [rgb[0], rgb[1], rgb[2], ALPHA_LEVELS[Math.min(3, level)]]);
}

/** Pale poured concrete, the material every ground slab in this region is made of. */
const CONCRETE_PALE = Object.freeze([[74, 68, 80], [96, 90, 100], [120, 113, 124], [146, 139, 150]]);
/** Chemical/oil staining: near-black with a rust bloom at its edge. */
const SPILL_DARK = Object.freeze([[14, 11, 18], [24, 19, 30], [38, 30, 44]]);
const SPILL_RUST = Object.freeze([[92, 52, 44], [124, 72, 56]]);
/** Faded painted line marking on an apron. */
const PAINT_LINE = Object.freeze([158, 150, 138]);
/** The lit lip of a cast pad - one step above the palest concrete body. */
const CONCRETE_LIT = Object.freeze([168, 161, 170]);

/**
 * THE DOMINANT STRUCTURE: a turbine hall.
 *
 * 320 px of frontage - ten tiles, 1.7x the widest ruin in the kit - and the
 * only object in the region with a repeating architectural rhythm. Read from
 * left to right it is: a tall doorway that gives the whole region its human
 * scale, a long run of broken windows under a sawtooth north-light roof, and
 * then the collapsed third where the roof has come down into its own trusses.
 *
 * Sawtooth is chosen deliberately over a plain gable: a north-light roof is
 * unambiguously a WORKSHOP silhouette in a way no other roof is, so the hall
 * identifies itself at a glance even when it is only 40 px tall on screen.
 */
function paintPowerHallBody(scratch, spec, variant) {
  const kit = RUIN_CONCRETE;
  const seed = 71_000 + variant * 1279;
  const baseY = spec.pivot[1];
  const left = 8;
  const right = 232 + variant * 12;
  const wallTop = baseY - (66 + variant * 6);
  // The collapse begins here and the roof line falls away to the right.
  const breakAt = Math.round(left + (right - left) * (0.66 - variant * 0.05));

  const roofAt = (x) => {
    if (x <= breakAt) return wallTop;
    const fall = (x - breakAt) / Math.max(1, right - breakAt);
    return wallTop + Math.round(fall ** 1.35 * (baseY - wallTop) * 0.92);
  };

  // --- the shell -----------------------------------------------------------
  for (let x = left; x <= right; x += 1) {
    const top = roofAt(x);
    const collapsed = x > breakAt;
    for (let y = top; y <= baseY; y += 1) {
      const down = (y - top) / Math.max(1, baseY - top);
      let colour = ruinShade(kit, ((x - left) / (right - left) - 0.5) * 0.5, down * 1.00 - 0.86);
      // Poured lift courses.
      if ((baseY - y) % 13 === 0) colour = kit.shade;
      // Pilasters: the structural bay rhythm, every 26 px.
      if ((x - left) % 26 < 3) colour = down < 0.9 ? kit.face : kit.body;
      if (collapsed && hash2(chunky(x), chunky(y), seed + 3) > 0.62) colour = kit.shade;
      if (hash2(chunky(x), chunky(y), seed + 7) > 0.92) colour = kit.shade;
      put(scratch, x, y, colour);
    }
    // The wall head catches the light.
    if (!collapsed) put(scratch, x, top, kit.lit);
  }

  // --- the sawtooth north-light roof --------------------------------------
  const toothPitch = 30;
  for (let x = left; x <= breakAt; x += 1) {
    const phase = ((x - left) % toothPitch) / toothPitch;
    // Vertical glazed face on the left of each tooth, sloping back to the right.
    const rise = phase < 0.28
      ? Math.round(22 * (phase / 0.28))
      : Math.round(22 * (1 - (phase - 0.28) / 0.72));
    for (let y = wallTop - rise; y < wallTop; y += 1) {
      const up = (wallTop - y) / 22;
      let colour;
      if (phase < 0.28) {
        // Glazing: mostly gone, a few bars and shards left.
        const bar = Math.round(y / 4) % 3 === 0;
        colour = bar ? kit.body : kit.outline;
        if (hash2(chunky(x), chunky(y), seed + 11) > 0.90) colour = kit.lit;
      } else {
        colour = ruinShade(kit, 0.2, -0.30 - up * 0.35);
        if ((x + y) % 7 === 0) colour = kit.shade;
      }
      put(scratch, x, y, colour);
    }
    if (rise > 0) put(scratch, x, wallTop - rise, kit.lit);
  }

  // --- the window rhythm ---------------------------------------------------
  for (let bay = 0; bay < 14; bay += 1) {
    const wx = left + 8 + bay * 26;
    if (wx + 12 > breakAt) break;
    const wTop = wallTop + 14;
    const wBottom = baseY - 16;
    for (let y = wTop; y <= wBottom; y += 1) {
      for (let x = wx; x < wx + 12; x += 1) {
        // Glazing bars survive on some windows; most are just dark openings.
        const bar = (x - wx) % 6 === 0 || (y - wTop) % 9 === 0;
        const intact = hash2(bay, Math.round((y - wTop) / 9), seed + 17) > 0.55;
        put(scratch, x, y, bar && intact ? kit.body : kit.outline);
      }
    }
    put(scratch, wx - 1, wTop - 1, kit.lit);
  }

  // --- the tall doorway: the scale cue -------------------------------------
  const doorX = left + 4;
  for (let y = baseY - 40; y <= baseY; y += 1) {
    for (let x = doorX; x < doorX + 18; x += 1) {
      const edge = x === doorX || x === doorX + 17 || y === baseY - 40;
      put(scratch, x, y, edge ? kit.face : kit.outline);
    }
  }

  // --- the collapse: roof trusses down in the open end ---------------------
  for (let truss = 0; truss < 7; truss += 1) {
    const tx = breakAt + 6 + truss * 9;
    if (tx > right - 4) break;
    const lean = 0.6 + hash2(truss, variant, seed + 19) * 1.5;
    const len = 22 + Math.round(hash2(truss, variant, seed + 23) * 18);
    for (let s = 0; s < len; s += 1) {
      const x = Math.round(tx + s * (truss % 2 === 0 ? lean : -lean) * 0.5);
      const y = baseY - 6 - Math.round(s * 0.75);
      put(scratch, x, y, RUIN_STEEL.face);
      put(scratch, x + 1, y, RUIN_STEEL.shade);
      if (s % 6 === 0) put(scratch, x, y - 1, RUIN_RUST);
    }
  }
  // Rubble against the broken end.
  for (let s = 0; s < 120; s += 1) {
    const rx = Math.round(breakAt + hash2(s, variant, seed + 29) * (right - breakAt + 26));
    const ry = Math.round(baseY - hash2(s, variant, seed + 31) * 12);
    const rw = 2 + Math.round(hash2(s, variant, seed + 37) * 4);
    const colour = hash2(s, variant, seed + 41) > 0.72 ? kit.body : kit.shade;
    for (let x = 0; x < rw; x += 1) put(scratch, rx + x, ry, colour);
  }
  // Spalled plinth.
  for (let x = left; x <= right; x += 1) {
    if (hash2(chunky(x), 0, seed + 43) > 0.55) put(scratch, x, baseY, kit.shade);
  }
}

/**
 * A bunded tank farm: three squat storage cylinders behind a low containment
 * wall, one split down the flank and slumped into its own bund.
 *
 * The tank farm is the second-largest structure and it does a specific job the
 * towers cannot: it puts a REPEATED unit in the frame. Three of the same thing
 * standing together is what separates "a plant" from "a ruin".
 */
function paintTankFarmBody(scratch, spec, variant) {
  const kit = RUIN_STEEL;
  const seed = 72_000 + variant * 1301;
  const baseY = spec.pivot[1];
  const tanks = [
    { cx: 40, r: 27, h: 58 + variant * 6, split: false },
    { cx: 92, r: 23, h: 50 + variant * 4, split: true },
    { cx: 136, r: 19, h: 40, split: false },
  ];
  // The containment bund, drawn first so the tanks stand in it.
  for (let x = 6; x <= 162; x += 1) {
    for (let y = baseY - 9; y <= baseY; y += 1) {
      const down = (y - (baseY - 9)) / 9;
      let colour = ruinShade(RUIN_CONCRETE, 0, down * 1.2 - 0.5);
      if (hash2(chunky(x), chunky(y), seed + 3) > 0.90) colour = RUIN_CONCRETE.shade;
      put(scratch, x, y, colour);
    }
    put(scratch, x, baseY - 9, RUIN_CONCRETE.lit);
  }
  for (const [index, tank] of tanks.entries()) {
    const top = baseY - 6 - tank.h;
    for (let x = tank.cx - tank.r; x <= tank.cx + tank.r; x += 1) {
      const across = (x - tank.cx) / tank.r;
      // A split tank's shell is pinched inward where it tore.
      const pinch = tank.split && across > 0.15 && across < 0.72 ? 6 : 0;
      const crownDrop = Math.round(Math.abs(across) ** 2 * 7) + pinch;
      for (let y = top + crownDrop; y <= baseY - 6; y += 1) {
        const down = (y - top) / Math.max(1, baseY - 6 - top);
        let colour = ruinShade(kit, across * 1.25, down * 0.55 - 0.30);
        // Welded courses: a tank is built in rings.
        if ((y - top) % 11 === 0) colour = kit.shade;
        // Rust runs bleeding down from the rim.
        if (hash2(chunky(x), index, seed + 7) > 0.80 && down < 0.75) colour = RUIN_RUST;
        if (hash2(chunky(x), chunky(y), seed + 11) > 0.93) colour = kit.shade;
        put(scratch, x, y, colour);
      }
      // The lit crown ellipse.
      put(scratch, x, top + crownDrop, across < 0.1 ? kit.lit : kit.face);
    }
    if (tank.split) {
      // The tear itself: a hard dark vertical gash showing the empty interior.
      for (let y = top + 4; y <= baseY - 8; y += 1) {
        const w = 2 + Math.round(fbm(y * 0.12, index, seed + 13, 2) * 5);
        for (let x = 0; x < w; x += 1) {
          put(scratch, tank.cx + 6 + x, y, kit.outline);
        }
        put(scratch, tank.cx + 6 - 1, y, kit.lit);
      }
    }
  }
  // A short pipe run leaving the bund to the right - the farm connects to something.
  for (let x = 150; x < 178; x += 1) {
    for (let d = -3; d <= 3; d += 1) {
      const y = baseY - 20 + d;
      let colour = ruinShade(kit, 0, (d / 3) * 1.2);
      if (hash2(chunky(x), chunky(y), seed + 17) > 0.82) colour = RUIN_RUST;
      put(scratch, x, y, colour);
    }
  }
}
...
/**
 * CONNECTIVE TISSUE: an elevated pipe run on low trestles.
 *
 * The frames JOIN. Variants 0/1 draw the pipe from x=0 to x=31 at a fixed
 * height, so consecutive tiles form one unbroken line across the plain;
 * variant 2 does the same vertically. That is the whole point — a pipeline is
 * only industrial if it GOES somewhere, and a single pipe segment sitting alone
 * in a field is exactly the clutter this push is trying not to be.
 */
function paintPipeline(surface, ox, oy, spec, variant) {
  const kit = RUIN_STEEL;
  const seed = 73_000 + variant * 691;
  const { width, height } = spec;
  const shade = (nd, x, y) => {
    let colour = ruinShade(kit, 0, nd * 1.25);
    if (hash2(chunky(x), chunky(y), seed + 7) > 0.84) colour = RUIN_RUST;
    return colour;
  };
  const leg = (x, fromY) => {
    for (let y = fromY; y < height - 2; y += 1) {
      put(surface, ox + x, oy + y, kit.shade);
      put(surface, ox + x + 1, oy + y, kit.outline);
    }
    veil(surface, ox + x - 1, oy + height - 2, ACCENT.shadow, 0.5);
    veil(surface, ox + x + 2, oy + height - 2, ACCENT.shadow, 0.5);
  };

  if (variant === 2) {
    // North-south: the pipe runs away from the viewer, full frame height. It
    // needs MORE light than the east-west run, not less: seen end-on there is
    // no silhouette to read, so without a lit flank it collapses into a dark
    // bar that reads as a post rather than a pipe.
    for (let y = 0; y < height; y += 1) {
      for (let d = -6; d <= 6; d += 1) {
        const nd = d / 6;
        let colour = ruinShade(kit, 0, nd * 1.05 - 0.30);
        if (hash2(chunky(16 + d), chunky(y), seed + 7) > 0.86) colour = RUIN_RUST;
        put(surface, ox + 16 + d, oy + y, colour);
      }
      put(surface, ox + 12, oy + y, kit.lit);
      put(surface, ox + 11, oy + y, kit.face);
    }
    // Bolted collars every frame, so a run has a rhythm rather than being a bar.
    for (let d = -8; d <= 8; d += 1) {
      const colour = Math.abs(d) > 6 ? kit.shade : kit.face;
      put(surface, ox + 16 + d, oy + 8, colour);
      put(surface, ox + 16 + d, oy + 9, kit.shade);
    }
    // A trestle foot on the shaded flank, and its shadow on the ground.
    for (let y = 20; y < height - 2; y += 1) put(surface, ox + 24, oy + y, kit.shade);
    veil(surface, ox + 25, oy + height - 3, ACCENT.shadow, 0.5);
    veil(surface, ox + 26, oy + height - 3, ACCENT.shadow, 0.4);
    return;
  }
  if (variant === 3) {
    // A riser terminating the run: a vertical stub, a flange and a valve wheel.
    for (let y = 6; y < height - 4; y += 1) {
      for (let d = -4; d <= 4; d += 1) put(surface, ox + 16 + d, oy + y, shade(d / 4, 16 + d, y));
    }
    for (let x = 10; x <= 22; x += 1) {
      put(surface, ox + x, oy + 12, kit.face);
      put(surface, ox + x, oy + 13, kit.shade);
    }
    // The wheel: a ring with four spokes. Small, but unmistakably a valve.
    for (let a = 0; a < 32; a += 1) {
      const t = (a / 32) * Math.PI * 2;
      put(surface, ox + 16 + Math.round(Math.cos(t) * 6), oy + 6 + Math.round(Math.sin(t) * 3), kit.face);
    }
    for (let s = -6; s <= 6; s += 1) put(surface, ox + 16 + s, oy + 6, kit.body);
    for (let s = -3; s <= 3; s += 1) put(surface, ox + 16, oy + 6 + s, kit.body);
    veil(surface, ox + 12, oy + height - 3, ACCENT.shadow, 0.55);
    return;
  }
  // East-west: the pipe spans the whole frame width, so runs are continuous.
  const pipeY = 16;
  leg(8, pipeY + 6);
  leg(23, pipeY + 6);
  for (let x = 0; x < width; x += 1) {
    for (let d = -5; d <= 5; d += 1) {
      put(surface, ox + x, oy + pipeY + d, shade(d / 5, x, pipeY + d));
    }
    put(surface, ox + x, oy + pipeY - 4, kit.lit);
  }
  if (variant === 1) {
    // A bolted joint collar: the detail that makes a long pipe read as built.
    for (let x = 13; x <= 18; x += 1) {
      for (let d = -7; d <= 7; d += 1) {
        put(surface, ox + x, oy + pipeY + d, Math.abs(d) > 5 ? kit.shade : kit.face);
      }
    }
    for (let d = -6; d <= 6; d += 2) put(surface, ox + 15, oy + pipeY + d, kit.outline);
  }
  for (let x = 0; x < width; x += 1) {
    veil(surface, ox + x, oy + height - 3, ACCENT.shadow, 0.30);
  }
}

/** CONNECTIVE TISSUE: a perimeter fence. Frames join at 32 px, so it runs. */
function paintFence(surface, ox, oy, spec, variant) {
  const kit = RUIN_STEEL;
  const seed = 74_000 + variant * 733;
  const { width, height } = spec;
  const post = (x, topY) => {
    for (let y = topY; y < height - 2; y += 1) {
      put(surface, ox + x, oy + y, kit.face);
      put(surface, ox + x + 1, oy + y, kit.shade);
    }
    veil(surface, ox + x + 2, oy + height - 2, ACCENT.shadow, 0.5);
  };
  const mesh = (x0, x1, topY, bottomY, gapAt) => {
    for (let y = topY; y <= bottomY; y += 1) {
      for (let x = x0; x <= x1; x += 1) {
        if (gapAt !== null && x > gapAt && x < gapAt + 9) continue;
        // Chain link reads as a sparse diagonal weave, never a solid panel.
        if ((x + y) % 4 !== 0 && (x - y) % 4 !== 0) continue;
        if (hash2(chunky(x), chunky(y), seed + 5) > 0.62) continue;
        setPixel(surface, ox + x, oy + y, [...kit.body, ALPHA_LEVELS[2]]);
      }
    }
    for (const y of [topY, bottomY]) {
      for (let x = x0; x <= x1; x += 1) put(surface, ox + x, oy + y, kit.shade);
    }
  };

  if (variant === 1) {
    // North-south: one post, wire running off both the top and bottom edges.
    post(15, 6);
    for (let y = 0; y < height - 2; y += 1) {
      if (y % 3 === 0) put(surface, ox + 14, oy + y, kit.shade);
    }
    mesh(10, 21, 8, height - 4, null);
    return;
  }
  const torn = variant === 2;
  post(2, torn ? 9 : 5);
  post(18, 5);
  mesh(2, width - 1, torn ? 11 : 7, height - 4, torn ? 22 : null);
}

/** CONNECTIVE TISSUE: a rail spur at ground level. Flat, joins at 32 px. */
function paintRailSpur(surface, ox, oy, spec, variant) {
  const seed = 75_000 + variant * 787;
  const { width, height } = spec;
  const sleeper = [46, 38, 42];
  const rail = RUIN_STEEL.face;
  if (variant === 1) {
    for (let y = 0; y < height; y += 1) {
      if (y % 6 === 0) {
        for (let x = 6; x < 26; x += 1) stain(surface, ox + x, oy + y, sleeper, 3);
      }
    }
    for (const x of [11, 21]) {
      for (let y = 0; y < height; y += 1) {
        stain(surface, ox + x, oy + y, hash2(x, y, seed) > 0.85 ? RUIN_RUST : rail, 3);
      }
    }
    return;
  }
  for (let x = 0; x < width; x += 1) {
    if (x % 6 === 0) {
      for (let y = 4; y < 15; y += 1) stain(surface, ox + x, oy + y, sleeper, 3);
    }
  }
  for (const y of [7, 12]) {
    for (let x = 0; x < width; x += 1) {
...
/**
 * GROUND EVIDENCE: the foundation slab of a building that is no longer there.
 *
 * The cheapest industrial signal in the whole kit — no silhouette, no shadow,
 * no height, and it still says "a structure stood here" instantly, because
 * nothing in nature is a rectangle. Drawn flat and translucent so the terrain's
 * own crazing and ash reads straight through it.
 */
function paintSlab(surface, ox, oy, spec, variant) {
  const seed = 76_000 + variant * 811;
  const left = 2;
  const right = spec.width - 3;
  const top = 5;
  const bottom = spec.height - 3;
  // Variant 1 is L-shaped; variant 2 carries a raised machine plinth.
  const notch = variant === 1 ? { x: right - 20, y: top + 12 } : null;
  for (let y = top; y <= bottom; y += 1) {
    for (let x = left; x <= right; x += 1) {
      if (notch !== null && x > notch.x && y < notch.y) continue;
      // The slab edge is broken, not ruled: erode it with noise.
      const edge = Math.min(x - left, right - x, y - top, bottom - y);
      if (edge < 2 && hash2(chunky(x), chunky(y), seed + 3) > 0.62) continue;
      let level = 3;
      let colour = CONCRETE_PALE[3];
      if (hash2(chunky(x), chunky(y), seed + 7) > 0.72) colour = CONCRETE_PALE[2];
      // A hard lit lip on the north and west edges: this is a raised pad, and
      // the lip is what makes a rectangle read as CAST rather than painted.
      if (x - left < 2 || y - top < 2) colour = CONCRETE_LIT;
      // Expansion joints: the grid a poured floor is always cast in.
      if ((x - left) % 17 === 0 || (y - top) % 15 === 0) colour = CONCRETE_PALE[1];
      // A shadowed lip on the south and east edges completes the relief.
      if (right - x < 2 || bottom - y < 2) colour = CONCRETE_PALE[0];
      // A structural crack wandering across it.
      const crack = Math.abs(fbm(x * 0.06, y * 0.09, seed + 11, 2) - 0.5);
      if (crack < 0.020) { colour = SPILL_DARK[2]; level = 3; }
      stain(surface, ox + x, oy + y, colour, level);
    }
  }
  // Rebar stubs along one broken edge - the detail that dates it as concrete.
  for (let s = 0; s < 9; s += 1) {
    const rx = left + 3 + Math.round(hash2(s, variant, seed + 13) * (right - left - 6));
    for (let y = bottom - 2; y <= bottom + 1; y += 1) {
      stain(surface, ox + rx, oy + y, SPILL_RUST[1], 3);
    }
  }
  if (variant === 2) {
    for (let y = top + 8; y < top + 22; y += 1) {
      for (let x = left + 12; x < left + 34; x += 1) {
        stain(surface, ox + x, oy + y, CONCRETE_PALE[3], 3);
      }
    }
  }
}
...
/**
 * GROUND EVIDENCE: a scorched concrete apron with faded line markings.
 *
 * THE EDGE IS RECTANGULAR, AND THAT IS THE WHOLE DESIGN. The first cut gave the
 * apron an organic eroded edge, which was correct for a worn slab and completely
 * wrong here: this region's own `ash` / `ashpale` tiers ARE large pale patches,
 * so a pale organic blob is indistinguishable from drifted ash at 1:1. Concrete
 * reads as concrete because of straight edges, expansion joints and a cast lip —
 * never because of its value. Corners are broken away and the edge is nibbled,
 * but the underlying figure stays a rectangle.
 */
function paintApron(surface, ox, oy, spec, variant) {
  const seed = 77_000 + variant * 857;
  const left = 3;
  const right = spec.width - 4;
  const top = 4;
  const bottom = spec.height - 4;
  // Two corners are broken off, chosen per variant, so no two aprons repeat.
  const broken = variant === 0
    ? [{ x: right, y: bottom, r: 22 }, { x: left, y: top, r: 13 }]
    : [{ x: left, y: bottom, r: 18 }, { x: right, y: top, r: 24 }];
  for (let y = top; y <= bottom; y += 1) {
    for (let x = left; x <= right; x += 1) {
      if (broken.some(({ x: bx, y: by, r }) => (
        Math.hypot(x - bx, (y - by) * 1.5) < r * (0.65 + fbm(x * 0.09, y * 0.11, seed + 3, 2) * 0.7)
      ))) continue;
      const edge = Math.min(x - left, right - x, y - top, bottom - y);
      if (edge < 2 && hash2(chunky(x), chunky(y), seed + 5) > 0.66) continue;
      let colour = CONCRETE_PALE[2];
      if (hash2(chunky(x), chunky(y), seed + 7) > 0.74) colour = CONCRETE_PALE[3];
      if (hash2(chunky(x), chunky(y), seed + 9) > 0.92) colour = CONCRETE_PALE[1];
      // Expansion joints: a poured apron is cast in bays, and the ruled grid is
      // the second thing (after the straight edge) that says "not ash".
      if ((x - left) % 21 === 0 || (y - top) % 19 === 0) colour = CONCRETE_PALE[0];
      // Scorching, heaviest toward the fissure side.
      const scorch = clamp01((x / spec.width - 0.42) * 1.7)
        * (0.5 + fbm(x * 0.04, y * 0.06, seed + 11, 2) * 0.9);
      if (scorch > 0.66) colour = SCORCH[1];
      else if (scorch > 0.44) colour = SPILL_DARK[2];
      const crack = Math.abs(fbm(x * 0.05, y * 0.08, seed + 13, 2) - 0.5);
      if (crack < 0.017) colour = SPILL_DARK[0];
      // The cast lip: lit on the key side, shadowed opposite.
      if (x - left < 2 || y - top < 2) colour = CONCRETE_LIT;
      if (right - x < 2 || bottom - y < 2) colour = CONCRETE_PALE[0];
      stain(surface, ox + x, oy + y, colour, 3);
    }
  }
  if (variant === 0) {
    // Two faded painted lines - a bay marking. Nothing else in this world has a
    // straight painted line on the ground.
    for (const ly of [top + 14, bottom - 14]) {
      for (let x = left + 8; x < right - 8; x += 1) {
        if (hash2(x, ly, seed + 17) > 0.58) continue;
        stain(surface, ox + x, oy + ly, PAINT_LINE, 2);
      }
    }
  }
}

/** GROUND EVIDENCE: a chemical spill stain with a rust bloom at its margin. */
function paintSpill(surface, ox, oy, spec, variant) {
  const seed = 78_000 + variant * 883;
  const cx = spec.width * (0.40 + variant * 0.06);
  const cy = spec.height * 0.55;
  for (let y = 0; y < spec.height; y += 1) {
    for (let x = 0; x < spec.width; x += 1) {
      const dx = (x - cx) / (spec.width * 0.42);
      const dy = (y - cy) / (spec.height * 0.40);
      // A spill runs downhill: stretched, lobed, never a disc.
      const r = Math.hypot(dx, dy * 1.25) + fbm(x * 0.07, y * 0.10, seed, 3) * 0.55 - 0.24;
      if (r > 1) continue;
      let colour = SPILL_DARK[0];
      let level = 3;
      // The rust bloom is DIRECTIONAL - it wicks out on the downhill/right
      // margin only. A rust ring all the way round read as a doughnut at 1:1,
      // which is the exact failure the pilot's own glow pool was sent back for.
      const bloom = clamp01((dx + dy * 0.4) * 1.1)
        * (0.45 + fbm(x * 0.11, y * 0.13, seed + 21, 2) * 1.1);
      if (r > 0.55 && bloom > 0.52) colour = SPILL_RUST[0];
      if (r > 0.78 && bloom > 0.66) { colour = SPILL_RUST[1]; level = 2; }
      else if (r > 0.34 && hash2(chunky(x), chunky(y), seed + 5) > 0.72) colour = SPILL_DARK[2];
      if (r > 0.80 && bloom <= 0.52) level = 2;
      // A dried crust in the middle where it evaporated.
      if (r < 0.30 && fbm(x * 0.12, y * 0.14, seed + 9, 2) > 0.58) colour = SPILL_DARK[2];
      stain(surface, ox + x, oy + y, colour, level);
    }
  }
}

/** GROUND EVIDENCE with a body: a slag/tailings spoil heap and its spill fan. */
function paintTailingsBody(surface, ox, oy, spec, variant) {
  const seed = 79_000 + variant * 907;
  const baseY = spec.pivot[1];
  const peakX = 36 + variant * 4;
  const halfWidth = 34 + variant * 4;
  const height = 30 + variant * 6;
  // The spill fan first, flat on the ground to the right, so the heap sits on it.
  for (let x = peakX - 6; x < spec.width - 2; x += 1) {
    const run = (x - (peakX - 6)) / (spec.width - peakX - 4);
    const spread = 4 + run * 12;
    for (let d = -spread; d <= spread; d += 1) {
      const y = Math.round(baseY - 1 + d * 0.30);
      if (hash2(chunky(x), chunky(y), seed + 3) > 0.55 - run * 0.2) continue;
      // Level 2, deliberately: the fan is a thin wash of spilled fines, not a wall. At
      // the top alpha level it would be FULLY OPAQUE and `measureGroundContact` would
      // read it as ground contact, giving the heap a collision footprint two tiles to
      // the right of the heap itself.
      stain(surface, ox + x, oy + y, run > 0.55 ? DUST[0] : CLINKER[1], 2);
    }
  }
  for (let x = peakX - halfWidth; x <= peakX + halfWidth; x += 1) {
    const across = (x - peakX) / halfWidth;
    // A tipped spoil heap has a straight angle of repose, not a smooth dome.
    const crest = Math.round(height * (1 - Math.abs(across) ** 1.18)
      + fbm(x * 0.18, variant, seed + 5, 2) * 5 - 2);
    if (crest <= 0) continue;
    for (let y = baseY - crest; y <= baseY; y += 1) {
      const down = (baseY - y) / Math.max(1, crest);
      let index = Math.round(clamp01(0.30 + down * 0.55 - across * 0.30) * 3);
      if (hash2(chunky(x), chunky(y), seed + 7) > 0.72) index = Math.max(0, index - 1);
      let colour = CLINKER[Math.max(0, Math.min(3, index))];
      // Angular slag chunks catching the key light.
      if (hash2(chunky(x), chunky(y), seed + 11) > 0.94) colour = CLINKER[3];
      if (hash2(chunky(x), chunky(y), seed + 13) > 0.975) colour = CLINKER_RUST;
      put(surface, ox + x, oy + y, colour);
    }
    // The lit crest line.
    put(surface, ox + x, oy + baseY - crest, across < 0 ? SCREE[3] : SCREE[2]);
  }
}
...
/**
 * Measure where a drawn species actually TOUCHES THE GROUND, in frame-local pixels.
 *
 * This is published per frame so the region never has to hand-declare a contact model in
 * fractions and then keep it in sync with art that gets retuned. The defect it removes is
 * the one both region pilots recorded independently: a ruin BLOCKS one tile while being
 * DRAWN two to five tiles wide, so beings walk through the skirt of a cooling tower.
 *
 * Measured, not guessed, and the two exclusions are the whole reason it has to be measured:
 *
 *  - only FULLY OPAQUE pixels count. A cast shadow and a contact pool are laid with
 *    quantised alpha, so they are picture, not wall — a being may stand on a shadow.
 *  - only the band immediately above the object's lowest solid row counts. A cooling
 *    tower's flared rim is 60 px across at the top and 46 px across where it meets the
 *    ground; the ground is what collision is about.
 *
 * A flat species (a rail spur, a ground decal) has no wall at all, so it publishes a
 * zero-height contact and the region can skip it without a special case.
 *
 * @returns `{ x, y, width, height }` in frame-local pixels; width/height are 0 when the
 *   frame has no solid ground contact.
 */
function measureGroundContact(surface, ox, oy, spec) {
  const { width, height } = spec;
  const solid = (x, y) => surface.data[((oy + y) * surface.width + (ox + x)) * 4 + 3] === 255;
  let footRow = -1;
  for (let y = height - 1; y >= 0; y -= 1) {
    for (let x = 0; x < width; x += 1) {
      if (solid(x, y)) { footRow = y; break; }
    }
    if (footRow >= 0) break;
  }
  if (footRow < 0 || spec.flat === true) return { x: 0, y: 0, width: 0, height: 0 };
  const band = Math.max(2, Math.round(height * 0.05));
  const topRow = Math.max(0, footRow - band);
  let minX = width;
  let maxX = -1;
  for (let y = topRow; y <= footRow; y += 1) {
    for (let x = 0; x < width; x += 1) {
...
function buildSceneryAtlas() {
  ...
  for (const spec of SCENERY_SPECS) {
    for (let variant = 0; variant < spec.variants; variant += 1) {
      ...
      // Ruins size their own contact pool from their measured footprint;
      // linear species draw a tight shadow under their own posts; flat ground
      // evidence has no height at all and must never cast anything.
      const carriesOwnShadow = spec.id === "causeway" || spec.id === "emberwisp"
        || spec.id === "ashripple" || spec.ruin === true
        || spec.linear === true || spec.flat === true;
      if (!carriesOwnShadow) {
      ...
      switch (spec.id) {
        ...
        case "causeway":
          ...
          break;
        case "powerhall":
          paintRuin(surface, ox, oy, spec, variant, paintPowerHallBody, 0.60);
          break;
        case "tankfarm":
          paintRuin(surface, ox, oy, spec, variant, paintTankFarmBody, 0.58);
          break;
        case "pipeline":
          paintPipeline(surface, ox, oy, spec, variant);
          break;
        case "fence":
          paintFence(surface, ox, oy, spec, variant);
          break;
        case "railspur":
          paintRailSpur(surface, ox, oy, spec, variant);
          break;
        case "tailings":
          paintTailingsBody(surface, ox, oy, spec, variant);
          break;
        case "coolingtower":
      ...
      records.push({
        ...
        contact: measureGroundContact(surface, ox, oy, spec),
      });
      cursorX += spec.width;
...
export async function authorNirvanaWestProductionArt(outputRoot = DEFAULT_OUTPUT_ROOT) {
  ...
  const atlas = {
    ...
    /**
     * `[id, x, y, width, height, pivotX, pivotY, contactX, contactY, contactW, contactH]`
     *
     * The last four fields are the MEASURED ground contact (see `measureGroundContact`),
     * in frame-local pixels. They are what lets a region derive a real per-species
     * footprint instead of blocking one tile under a five-tile-wide object. A zero-area
     * contact means the species has no wall — a flat decal or a ground marking.
     */
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  authorNirvanaWestProductionArt()
    .then(async (atlas) => {
